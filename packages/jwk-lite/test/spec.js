...
describe('generateKey', () => {
  ['RS256', 'RS384', 'RS512', 'PS256', 'PS384', 'PS512'].map(algo => {
    supports({ generateKey: algo }).it(`generates an ${algo}`, () =>
      jwk.generateKey(algo)
      .then(res => {
        ...
        expect(res.publicKey.algorithm.name).toEqual(algorithms[algo].name)
        expect(res.publicKey.algorithm.hash).toBeDefined()
        expect(res.publicKey.algorithm.hash.name).toEqual(algorithms[algo].hash.name)
        ...
        expect(res.privateKey.algorithm.name).toEqual(algorithms[algo].name)
        expect(res.privateKey.algorithm.hash).toBeDefined()
        expect(res.privateKey.algorithm.hash.name).toEqual(algorithms[algo].hash.name)
  ...
  ['HS256', 'HS384', 'HS512'].map(algo => {
    supports({ generateKey: algo }).it(`generates an ${algo}`, () =>
      ...
      .then(res => {
        ...
        expect(res.sharedKey.type).toEqual('secret')
        expect(res.sharedKey.extractable).toEqual(true)
        expect(res.sharedKey.usages).toEqual(['sign', 'verify'])
        expect(res.sharedKey.algorithm).toBeDefined()
        expect(res.sharedKey.algorithm.name).toEqual(algorithms[algo].name)
        expect(res.sharedKey.algorithm.hash).toBeDefined()
        expect(res.sharedKey.algorithm.hash.name).toEqual(algorithms[algo].hash.name)
      })
    )
  });
  ...
  ['ES256', 'ES384', 'ES512'].map(algo => {
    supports({ generateKey: algo }).it(`generates an ${algo}`, () =>
      ...
      .then(res => {
        ...
        expect(res.publicKey.algorithm).toBeDefined()
        expect(res.publicKey.algorithm.name).toEqual(algorithms[algo].name)
        expect(res.publicKey.algorithm.namedCurve).toEqual(algorithms[algo].namedCurve)
        ...
        expect(res.privateKey.algorithm).toBeDefined()
        expect(res.privateKey.algorithm.name).toEqual(algorithms[algo].name)
        expect(res.privateKey.algorithm.namedCurve).toEqual(algorithms[algo].namedCurve)
      })
    )
  });
})
...
describe('importKey', () => {
  ...
  ['ES256', 'ES384', 'ES512'].map(algo => {
    supports({ importKey: algo }).it(`imports an ${algo}`, () =>
      jwk.importKey(keys[algo].private, { alg: algo })
      .then(res => {
        expect(res).toBeDefined();
        expect(res.type).toEqual('private')
        expect(res.extractable).toEqual(true)
        ...
        expect(res.algorithm).toBeDefined()
        expect(res.algorithm.name).toEqual(algorithms[algo].name)
        expect(res.algorithm.namedCurve).toEqual(algorithms[algo].namedCurve)
      })
    )
  });
});
